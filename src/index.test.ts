...
describe("toolExecuteBefore", () => {
  ...
  it("should prefix simple command with snip", async () => {
    ...
    expect(mockOutput.args.command).toBe(
      "SNIP_QUIET_NO_FILTER=1 snip go test ./...",
    );
  });
  ...
  it("should handle command with one env var prefix", async () => {
    ...
    await toolExecuteBefore(mockInput, mockOutput);
    expect(mockOutput.args.command).toBe(
      "CGO_ENABLED=0 SNIP_QUIET_NO_FILTER=1 snip go test ./...",
    );
  });
  ...
  it("should handle command with multiple env var prefixes", async () => {
    ...
    await toolExecuteBefore(mockInput, mockOutput);
    expect(mockOutput.args.command).toBe(
      "CGO_ENABLED=0 GOOS=linux SNIP_QUIET_NO_FILTER=1 snip go test ./...",
    );
  });
  ...
  it("should handle command with &&", async () => {
    ...
    await toolExecuteBefore(mockInput, mockOutput);
    expect(mockOutput.args.command).toBe(
      "SNIP_QUIET_NO_FILTER=1 snip go test && SNIP_QUIET_NO_FILTER=1 snip go build",
    );
  });
  ...
  it("should handle command with |", async () => {
    ...
    await toolExecuteBefore(mockInput, mockOutput);
    expect(mockOutput.args.command).toBe(
      "SNIP_QUIET_NO_FILTER=1 snip git log | head",
    );
  });
  ...
  it("should handle command with ||", async () => {
    ...
    await toolExecuteBefore(mockInput, mockOutput);
    expect(mockOutput.args.command).toBe(
      "SNIP_QUIET_NO_FILTER=1 snip test -f foo.txt || SNIP_QUIET_NO_FILTER=1 snip echo missing",
    );
  });
  ...
  it("should handle command with &", async () => {
    ...
    await toolExecuteBefore(mockInput, mockOutput);
    expect(mockOutput.args.command).toBe(
      "SNIP_QUIET_NO_FILTER=1 snip sleep 1 & SNIP_QUIET_NO_FILTER=1 snip sleep 2 &",
    );
  });
  ...
  it("should handle mixed operators", async () => {
    ...
    await toolExecuteBefore(mockInput, mockOutput);
    expect(mockOutput.args.command).toBe(
      "SNIP_QUIET_NO_FILTER=1 snip go test && SNIP_QUIET_NO_FILTER=1 snip go build; SNIP_QUIET_NO_FILTER=1 snip go run",
    );
  });
  ...
  it("should handle env vars with operators", async () => {
    ...
    await toolExecuteBefore(mockInput, mockOutput);
    expect(mockOutput.args.command).toBe(
      "FOO=bar SNIP_QUIET_NO_FILTER=1 snip go test && SNIP_QUIET_NO_FILTER=1 snip go build",
    );
  });
  ...
  describe("unproxyable shell builtins", () => {
    ...
    it("should skip cd but snip chained command", async () => {
      ...
      await toolExecuteBefore(mockInput, mockOutput);
      expect(mockOutput.args.command).toBe(
        "cd /tmp && SNIP_QUIET_NO_FILTER=1 snip ls",
      );
    });

    it("should not wrap do keyword in one-liner loop", async () => {
      mockOutput.args.command = 'for f in *.txt; do cat "$f"; done';
      await toolExecuteBefore(mockInput, mockOutput);
      expect(mockOutput.args.command).toBe('for f in *.txt; do cat "$f"; done');
    });
  });

  describe("already wrapped detection", () => {
    it("should detect quiet-mode wrapped command", async () => {
      mockOutput.args.command = "SNIP_QUIET_NO_FILTER=1 snip go test";
      await toolExecuteBefore(mockInput, mockOutput);
      expect(mockOutput.args.command).toBe(
        "SNIP_QUIET_NO_FILTER=1 snip go test",
      );
  ...
  describe("redirections with &", () => {
    it("should not break 2>&1 redirection", async () => {
      ...
      await toolExecuteBefore(mockInput, mockOutput);
      expect(mockOutput.args.command).toBe(
        'SNIP_QUIET_NO_FILTER=1 snip find / -name "*.log" 2>&1',
      );
    });
    ...
    it("should not break 1>&2 redirection", async () => {
      ...
      await toolExecuteBefore(mockInput, mockOutput);
      expect(mockOutput.args.command).toBe(
        "SNIP_QUIET_NO_FILTER=1 snip cmd 1>&2",
      );
    });
    ...
    it("should handle 2>&1 with pipe", async () => {
      ...
      await toolExecuteBefore(mockInput, mockOutput);
      expect(mockOutput.args.command).toBe(
        'SNIP_QUIET_NO_FILTER=1 snip find / -name "*.log" 2>&1 | grep error',
      );
    });
    ...
    it("should handle 2>&1 with chained commands", async () => {
      ...
      await toolExecuteBefore(mockInput, mockOutput);
      expect(mockOutput.args.command).toBe(
        "SNIP_QUIET_NO_FILTER=1 snip cmd1 2>&1 && SNIP_QUIET_NO_FILTER=1 snip cmd2",
      );
    });
  });
  ...
  describe("pipe expressions with quotes", () => {
    it("should not split pipes inside single quotes", async () => {
      ...
      await toolExecuteBefore(mockInput, mockOutput);
      expect(mockOutput.args.command).toBe(
        "SNIP_QUIET_NO_FILTER=1 snip cat file.json | jq '.content | .text'",
      );
    });
    ...
    it("should handle jq with fromjson", async () => {
      ...
      await toolExecuteBefore(mockInput, mockOutput);
      expect(mockOutput.args.command).toBe(
        "SNIP_QUIET_NO_FILTER=1 snip cat file.json | jq '.content[0].text | fromjson'",
      );
    });
    ...
    it("should handle multiple pipes in jq", async () => {
      ...
      await toolExecuteBefore(mockInput, mockOutput);
      expect(mockOutput.args.command).toBe(
        "SNIP_QUIET_NO_FILTER=1 snip cat file.json | jq '.a | .b | .c'",
      );
    });
    ...
    it("should handle pipe with || operator", async () => {
      ...
      await toolExecuteBefore(mockInput, mockOutput);
      expect(mockOutput.args.command).toBe(
        "SNIP_QUIET_NO_FILTER=1 snip cmd1 || SNIP_QUIET_NO_FILTER=1 snip cmd2",
      );
    });
    ...
    it("should handle mixed quotes and pipes", async () => {
      ...
      await toolExecuteBefore(mockInput, mockOutput);
      expect(mockOutput.args.command).toBe(
        'SNIP_QUIET_NO_FILTER=1 snip echo "hello | world" | cat',
      );
    });
  });
});
