...
describe("toolExecuteBefore", () => {
  ...
  it("should prefix simple command with snip", async () => {
    ...
    expect(mockOutput.args.command).toBe("SNIP_QUIET_NO_FILTER=1 snip go test ./...")
  })

  it("should handle command with one env var prefix", async () => {
    mockOutput.args.command = "CGO_ENABLED=0 go test ./..."
    await toolExecuteBefore(mockInput, mockOutput)
    expect(mockOutput.args.command).toBe("CGO_ENABLED=0 SNIP_QUIET_NO_FILTER=1 snip go test ./...")
  })

  it("should handle command with multiple env var prefixes", async () => {
    mockOutput.args.command = "CGO_ENABLED=0 GOOS=linux go test ./..."
    await toolExecuteBefore(mockInput, mockOutput)
    expect(mockOutput.args.command).toBe("CGO_ENABLED=0 GOOS=linux SNIP_QUIET_NO_FILTER=1 snip go test ./...")
  })

  it("should handle command with &&", async () => {
    mockOutput.args.command = "go test && go build"
    await toolExecuteBefore(mockInput, mockOutput)
    expect(mockOutput.args.command).toBe("SNIP_QUIET_NO_FILTER=1 snip go test && SNIP_QUIET_NO_FILTER=1 snip go build")
  })

  it("should handle command with |", async () => {
    mockOutput.args.command = "git log | head"
    await toolExecuteBefore(mockInput, mockOutput)
    expect(mockOutput.args.command).toBe("SNIP_QUIET_NO_FILTER=1 snip git log | head")
  })
  ...
  it("should handle command with ||", async () => {
    ...
    await toolExecuteBefore(mockInput, mockOutput)
    expect(mockOutput.args.command).toBe("SNIP_QUIET_NO_FILTER=1 snip test -f foo.txt || SNIP_QUIET_NO_FILTER=1 snip echo missing")
  })

  it("should handle command with &", async () => {
    mockOutput.args.command = "sleep 1 & sleep 2 &"
    await toolExecuteBefore(mockInput, mockOutput)
    expect(mockOutput.args.command).toBe("SNIP_QUIET_NO_FILTER=1 snip sleep 1 & SNIP_QUIET_NO_FILTER=1 snip sleep 2 &")
  })

  it("should handle mixed operators", async () => {
    mockOutput.args.command = "go test && go build; go run"
    await toolExecuteBefore(mockInput, mockOutput)
    expect(mockOutput.args.command).toBe("SNIP_QUIET_NO_FILTER=1 snip go test && SNIP_QUIET_NO_FILTER=1 snip go build; SNIP_QUIET_NO_FILTER=1 snip go run")
  })

  it("should handle env vars with operators", async () => {
    mockOutput.args.command = "FOO=bar go test && go build"
    await toolExecuteBefore(mockInput, mockOutput)
    expect(mockOutput.args.command).toBe("FOO=bar SNIP_QUIET_NO_FILTER=1 snip go test && SNIP_QUIET_NO_FILTER=1 snip go build")
  })
  ...
  describe("unproxyable shell builtins", () => {
    ...
    it("should skip cd but snip chained command", async () => {
      ...
      await toolExecuteBefore(mockInput, mockOutput)
      expect(mockOutput.args.command).toBe("cd /tmp && SNIP_QUIET_NO_FILTER=1 snip ls")
    })
  })
  ...
  describe("redirections with &", () => {
    it("should not break 2>&1 redirection", async () => {
      ...
      await toolExecuteBefore(mockInput, mockOutput)
      expect(mockOutput.args.command).toBe("SNIP_QUIET_NO_FILTER=1 snip find / -name \"*.log\" 2>&1")
    })

    it("should not break 1>&2 redirection", async () => {
      mockOutput.args.command = "cmd 1>&2"
      await toolExecuteBefore(mockInput, mockOutput)
      expect(mockOutput.args.command).toBe("SNIP_QUIET_NO_FILTER=1 snip cmd 1>&2")
    })

    it("should handle 2>&1 with pipe", async () => {
      mockOutput.args.command = "find / -name \"*.log\" 2>&1 | grep error"
      await toolExecuteBefore(mockInput, mockOutput)
      expect(mockOutput.args.command).toBe("SNIP_QUIET_NO_FILTER=1 snip find / -name \"*.log\" 2>&1 | grep error")
    })

    it("should handle 2>&1 with chained commands", async () => {
      mockOutput.args.command = "cmd1 2>&1 && cmd2"
      await toolExecuteBefore(mockInput, mockOutput)
      expect(mockOutput.args.command).toBe("SNIP_QUIET_NO_FILTER=1 snip cmd1 2>&1 && SNIP_QUIET_NO_FILTER=1 snip cmd2")
    })
  })
  ...
  describe("pipe expressions with quotes", () => {
    it("should not split pipes inside single quotes", async () => {
      ...
      await toolExecuteBefore(mockInput, mockOutput)
      expect(mockOutput.args.command).toBe("SNIP_QUIET_NO_FILTER=1 snip cat file.json | jq '.content | .text'")
    })
    ...
    it("should handle jq with fromjson", async () => {
      ...
      await toolExecuteBefore(mockInput, mockOutput)
      expect(mockOutput.args.command).toBe("SNIP_QUIET_NO_FILTER=1 snip cat file.json | jq '.content[0].text | fromjson'")
    })

    it("should handle multiple pipes in jq", async () => {
      mockOutput.args.command = "cat file.json | jq '.a | .b | .c'"
      await toolExecuteBefore(mockInput, mockOutput)
      expect(mockOutput.args.command).toBe("SNIP_QUIET_NO_FILTER=1 snip cat file.json | jq '.a | .b | .c'")
    })

    it("should handle pipe with || operator", async () => {
      mockOutput.args.command = "cmd1 || cmd2"
      await toolExecuteBefore(mockInput, mockOutput)
      expect(mockOutput.args.command).toBe("SNIP_QUIET_NO_FILTER=1 snip cmd1 || SNIP_QUIET_NO_FILTER=1 snip cmd2")
    })

    it("should handle mixed quotes and pipes", async () => {
      mockOutput.args.command = 'echo "hello | world" | cat'
      await toolExecuteBefore(mockInput, mockOutput)
      expect(mockOutput.args.command).toBe('SNIP_QUIET_NO_FILTER=1 snip echo "hello | world" | cat')
    })
  })
})